...
/*---------------------------------------------------------------------------*/
static int
GenerateTSOPacket(uint16_t core_id, uint16_t port,
				  const struct iovec *iov, const int iovcnt)
{
  struct rte_mbuf *m, *payloadm;
  struct rte_ether_hdr *ethh;
  struct rte_ipv4_hdr *iph;
  struct rte_tcp_hdr *tcph;
  int len = 0, hdrlen = 0, i;
  ...
  g_numBytesSent[core_id] += hdrlen + len;
	...
	buf_len = len + sizeof(struct rte_mbuf_ext_shared_info);
  ext_buf_addr = rte_malloc("External buffer", buf_len,
			RTE_CACHE_LINE_SIZE);
	CopyIOVToBuf(ext_buf_addr, iov, iovcnt);
	ret_shinfo = rte_pktmbuf_ext_shinfo_init_helper(ext_buf_addr, &buf_len,
		ext_buf_free_callback_fn, ext_buf_addr);
	if (ret_shinfo == NULL) {
		fprintf(stderr, "FAILED TO INIT SHARED INFO\n");
		exit(-1);
	}
	buf_iova = rte_malloc_virt2iova(ext_buf_addr);

  /* allocate an mbuf for sending */
  m = get_wptr(core_id, port, len);
  ethh = (struct rte_ether_hdr *)ext_buf_addr;
  assert(ethh != NULL);

  /* copy header data */
  // rte_memcpy((uint8_t *)ethh, iov[0].iov_base, iov[0].iov_len);

  /* set up some parameters */
  iph  = (struct rte_ipv4_hdr *)(ethh + 1);
  tcph = (struct rte_tcp_hdr *)(iph + 1);

  iph->total_length     = htons(len - ETHERNET_HEADER_LEN);
  ...
  m->ol_flags |= PKT_TX_IPV4 | PKT_TX_TCP_CKSUM |
	...
	if (m->ol_flags && EXT_ATTACHED_MBUF == 0) {
		fprintf(stderr, "FAILED TO ATTACH EXTERNAL MBUF\n");
		exit(-1);
	}

	dump_pkt(ext_buf_addr, len);
	...
	// for (i = 1; i < iovcnt; i++) {
	// 	// ext_buf_addr = iov[i].iov_base;
	// 	buf_len = iov[i].iov_len + sizeof(struct rte_mbuf_ext_shared_info);
	// 	ext_buf_addr = rte_malloc("External buffer", buf_len,
	// 		RTE_CACHE_LINE_SIZE);
	// 	// CopyIOVToBuf(payload, iov, iovcnt);
	// 	rte_memcpy(ext_buf_addr, iov[i].iov_base, iov[i].iov_len);
	// 	ret_shinfo = rte_pktmbuf_ext_shinfo_init_helper(ext_buf_addr, &buf_len,
	// 		ext_buf_free_callback_fn, ext_buf_addr);
	// 	if (ret_shinfo == NULL) {
	// 		fprintf(stderr, "FAILED TO INIT SHARED INFO\n");
	...
	// 	// payloadm->pkt_len = payloadm->data_len = iov[i].iov_len;
	// 	payloadm->nb_segs = 1;
	// 	payloadm->next    = NULL;

	// 	prev->next = payloadm;
	// 	m->pkt_len += iov[i].iov_len;
	// 	m->nb_segs += 1;

	// 	prev = payloadm;

	//   rte_pktmbuf_attach_extbuf(payloadm, ext_buf_addr, buf_iova, buf_len, ret_shinfo);

	// 	rte_pktmbuf_reset_headroom(payloadm);
	// 	rte_pktmbuf_adj(payloadm, iov[i].iov_len);
  ...
  // payloadm = get_wptr(core_id, port, len);
	// payloadm = rte_pktmbuf_alloc(pktmbuf_pool[core_id]);
	// payloadm->pkt_len = payloadm->data_len = len;
  // payloadm->nb_segs = 1;
  // payloadm->next    = NULL;
  // payload = (uint8_t *)rte_pktmbuf_mtod(payloadm, uint8_t *);
  // assert(payload != NULL);
  ...
  // m->nb_segs += 1;
	...
	// // fprintf(stderr, "Sending a packet with %u nb_segs and len %u\n", m->nb_segs, m->pkt_len);
  // payloadm->pkt_len = m->pkt_len;
  // payloadm->nb_segs = 1;
  
  // /* copy packet headers and payload to mbuf */
  // CopyIOVToBuf(payload, iov, iovcnt);

  *pbytes += hdrlen + len;
#define THRESHOLD_PACKETS (32 * MTU)
				   ...
				   double timestamp, uint64_t foff, int flen)
{
	...
	*/
	...
	while (flen > 0) {
		...
		if (GenerateTSOPacket(core_id, port, iov, cnt+1) <= 0) {
...
/*---------------------------------------------------------------------------*/
static FReadReq*
PrepareDiskRequest(struct file_cache *fc, uint16_t port,  uint8_t *pkthdr,
...
#define MAX_COMMAND 128                 /* max command length */
  ...
  rte_memcpy(ethh->d_addr.addr_bytes, fc->fc_haddr, sizeof(fc->fc_haddr));
	...
	if (fc->fc_nextSeq && TCP_SEQ_LT(ntohl(tcph->sent_seq), fc->fc_nextSeq)) {
		if(pread(fc->fc_fd, tmpbuf, flen-(flen%512), foff-(foff%512)) < 0) {
			TRACE_ERROR("pread() failed errno=%d at seq %u\n", errno, ntohl(tcph->sent_seq));
		}
	}
  ...
  frr = PrepareDiskRequest(fc, port, pktbuf, hdrlen, timestamp, foff, flen);
  if (frr != NULL)
...
/*---------------------------------------------------------------------------*/
static void
ResumeAfterDiskIO(DiskIOSlave *dis)
{
