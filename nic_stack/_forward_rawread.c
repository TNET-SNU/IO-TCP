...
static pool filecache_pools[MAX_CPUS];
...
OpenFileForOffload(int cid, char *file_name, uint32_t fid, uint8_t *haddr)
{
  ...
  fd_file_idx = rand() % 100000;
  lseek64(fd, 512000 * fd_file_idx, SEEK_SET);
...
/*---------------------------------------------------------------------------*/
static int
CreateIOVFromCache(struct iovec *iov,
				   ...
				   double timestamp, uint64_t foff, int flen)
{
	...
	*/
	...
	while (flen > 0) {
		...
		cnt = CreateIOVFromCache(&iov[1], fc, foff, len);
...
#define NUM_NVME 4
	...
	file_name[9] = (core_id % NUM_NVME) + '0';
        file_name[12] = 0;
