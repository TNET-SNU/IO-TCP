...
/*----------------------------------------------------------------------------*/
size_t 
SBPutEmpty(sb_manager_t sbm, struct tcp_send_buffer *buf, size_t len)
{
	size_t to_put;
	...
	if (buf->tail_off + to_put <= buf->size) {
		...
		/* if buffer overflows, move the existing payload and merge */
		// Check if offload_seq matches head_seq + len
		// If SBPut() increases len, then this will not match
		// and there's data in the bufffer
		if (buf->offload_seq_set &&
			buf->offload_seq != buf->head_seq + buf->len) {
			memmove(buf->data, buf->head, buf->len);
			buf->head = buf->data;
		}
		buf->head_off = 0;
