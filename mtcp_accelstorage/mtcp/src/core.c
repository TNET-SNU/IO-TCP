...
PrintThreadNetworkStats(mtcp_manager_t mtcp, struct net_stat *ns)
{
	...
	for (i = 0; i < CONFIG.eths_num; i++) {
		...
		ns->tx_offload_bytes[i] = mtcp->nstat.tx_offload_bytes[i] - mtcp->p_nstat.tx_offload_bytes[i];
		ns->mrtt[i] = mtcp->nstat.mrtt[i] - mtcp->p_nstat.mrtt[i];
		ns->mrtt_count[i] = mtcp->nstat.mrtt_count[i] - mtcp->p_nstat.mrtt_count[i];
#if NETSTAT_PERTHREAD
		if (CONFIG.eths[i].stat_print) {
			uint64_t ave_rtt = ns->mrtt_count[i] == 0 ? 0 : ns->mrtt[i]/ns->mrtt_count[i];
			fprintf(stderr, "[CPU%2d] %s flows: %6u, "
					"RX: %7ld(pps) (err: %5ld), %5.2lf(Gbps), "
					"TX: %7ld(pps), %5.2lf(Gbps), %5.2lf(Gbps), Ave RTT: %lu\n", 
					mtcp->ctx->cpu, CONFIG.eths[i].dev_name, mtcp->flow_cnt, 
					ns->rx_packets[i], ns->rx_errors[i], GBPS(ns->rx_bytes[i]), 
				ns->tx_packets[i], GBPS(ns->tx_bytes[i]), GBPS(ns->tx_offload_bytes[i]), ave_rtt);
		}
#endif
	}
#ifdef ENABLELRO
...
#define TIMEOUT 1
	...
	for (i = 0; i < CONFIG.num_cores; i++) {
		if (running[i]) {
			...
			for (j = 0; j < CONFIG.eths_num; j++) {
				...
				g_nstat.tx_offload_bytes[j] += ns.tx_offload_bytes[j];
				g_nstat.mrtt[j] += ns.mrtt[j];
				g_nstat.mrtt_count[j] += ns.mrtt_count[j];
			}
#ifdef ENABLELRO
	...
	for (i = 0; i < CONFIG.eths_num; i++) {
		if (CONFIG.eths[i].stat_print) {
			uint64_t ave_rtt = g_nstat.mrtt_count[i] == 0 ? 0 : g_nstat.mrtt[i]/g_nstat.mrtt_count[i];
			fprintf(stderr, "[ ALL ] %s flows: %6u, "
					...
					GBPS(g_nstat.tx_bytes[i]), GBPS(g_nstat.tx_offload_bytes[i]), ave_rtt);
		}
	}
#ifdef ENABLELRO
...
HandleApplicationCalls(mtcp_manager_t mtcp, uint32_t cur_ts)
{
	...
	/* offload queue handling */
	while ((stream = StreamDequeue(mtcp->offloadq))) {
		stream->sndvar->on_offloadq = FALSE;
		AddtoOffloadFileList(mtcp, stream);
	}
...
WritePacketsToChunks(mtcp_manager_t mtcp, uint32_t cur_ts)
{
	...
	if (mtcp->g_sender->offload_file_list_cnt)
		WriteTCPOffloadFileList(mtcp, mtcp->g_sender, cur_ts, thresh);
	...
	for (i = 0; i < CONFIG.eths_num; i++) {
		...
		if (mtcp->n_sender[i]->offload_file_list_cnt)
			WriteTCPOffloadFileList(mtcp, mtcp->n_sender[i], cur_ts, thresh);
...
CreateMTCPSender(int ifidx)
{
	...
	TAILQ_INIT(&sender->offload_file_list);
...
InitializeMTCPManager(struct mtcp_thread_context* ctx)
{
	...
	sprintf(pool_name, "ov_pool_%d", ctx->cpu);
	mtcp->ov_pool = MPCreate(pool_name, sizeof(struct offload_vars), 
			sizeof(struct offload_vars) * CONFIG.max_concurrency
			* MAX_OV_POOL_SIZE);
	if (!mtcp->ov_pool) {
		CTRACE_ERROR("Failed to allocate offload vars pool.\n");
		return NULL;
	}
	sprintf(pool_name, "ob_pool_%d", ctx->cpu);
	mtcp->ob_pool = MPCreate(pool_name, sizeof(struct offload_buffer), 
			sizeof(struct offload_buffer) * CONFIG.max_concurrency
			* MAX_OB_POOL_SIZE);
	if (!mtcp->ob_pool) {
		CTRACE_ERROR("Failed to allocate offload buffer pool.\n");
		return NULL;
	}
#else
	...
	mtcp->ov_pool = MPCreate(sizeof(struct offload_vars), 
			sizeof(struct offload_vars) * CONFIG.max_concurrency
			* MAX_OV_POOL_SIZE);
	if (!mtcp->ov_pool) {
		CTRACE_ERROR("Failed to allocate offload vars pool.\n");
	...
	mtcp->ob_pool = MPCreate(sizeof(struct offload_buffer), 
			sizeof(struct offload_buffer) * CONFIG.max_concurrency
			* MAX_OB_POOL_SIZE);
	if (!mtcp->ob_pool) {
		CTRACE_ERROR("Failed to allocate offload buffer pool.\n");
	...
	mtcp->offloadq = CreateStreamQueue(CONFIG.max_concurrency);
	if(!mtcp->offloadq) {
		CTRACE_ERROR("Failed to create offload queue.\n");
		return NULL;
	}
	...
	TAILQ_INIT(&mtcp->ov_cleanup_list);
	TAILQ_INIT(&mtcp->ob_cleanup_list);
#if USE_SPIN_LOCK
	if (pthread_spin_init(&mtcp->offload_cleanup_lock, PTHREAD_PROCESS_PRIVATE)) {
		perror("pthread_spin_init of offload_cleanup_lock");
#else
	if (pthread_mutex_init(&mtcp->offload_cleanup_lock, NULL)) {
...
MTCPRunThread(void *arg)
{
	...
	SQ_LOCK_INIT(&ctx->offloadq_lock, "ctx->offloadq_lock", exit(-1));
...
mtcp_free_context(mctx_t mctx)
{
	...
	SQ_LOCK_DESTROY(&ctx->offloadq_lock);
