...
#define TCP_RTO_MIN 0
	...
	mtcp->nstat.mrtt[0] += mrtt;
	mtcp->nstat.mrtt_count[0]++; 
...
TimeDiffMS(struct timeval* ptv)
{
  struct timeval now;

  gettimeofday(&now, NULL);

  return ((now.tv_sec - ptv->tv_sec) * 1000000
          + (now.tv_usec - ptv->tv_usec));
}
/*---------------------------------------------------------------------*/
static inline void ConsumeTime(long int microsecond)
{
  struct timeval start;

  gettimeofday(&start, NULL);

  while (TimeDiffMS(&start) < microsecond)
    ;
}
		...
		uint16_t window, int payloadlen)
{
	...
	if (rmlen > 0) {
		...
		if (cur_stream->saw_timestamp) {
			if (cur_stream->rcvvar->ts_lastack_rcvd >= cur_ts)  
				mrtt = 0;
			else
				mrtt = cur_ts - cur_stream->rcvvar->ts_lastack_rcvd;

			EstimateRTT(mtcp, cur_stream, mrtt);
		...
		// Remove sent bytes from offload buffers
		uint32_t to_remove;
		TAILQ_FOREACH_SAFE(ob, &sndvar->offload_buffer_list,
											 offload_buffer_link, _ob) {
			if (TCP_SEQ_GT(ack_seq, ob->head_seq)
				&& TCP_SEQ_GEQ(ob->head_seq + ob->len, ack_seq)) {
				to_remove = MIN(ack_seq - ob->head_seq, ob->len);
				ob->len -= to_remove;
				ob->head_seq += to_remove;
				ob->offset += to_remove;
			}
			if (ob->len == 0) {
				TAILQ_REMOVE(&sndvar->offload_buffer_list, ob, offload_buffer_link);
				SBUF_LOCK(&mtcp->offload_cleanup_lock);
				TAILQ_INSERT_TAIL(&mtcp->ob_cleanup_list, ob, ob_cleanup_link);
				SBUF_UNLOCK(&mtcp->offload_cleanup_lock);
			} else {
				// the last buffer still has content, so stop removing
				break;
			}
		}
		...
		// fprintf(stderr, "ProcessACK %d\n", __LINE__);

		TAILQ_FOREACH_SAFE(ov, &sndvar->offload_vars_list,
									offload_vars_link, _ov) {
			
			// Check for sending ACKD
			if (ov->offload_target_ackd) {
				// fprintf(stderr, "ProcessACK %d ack_seq:%d offload_target_ackd:%d \n", __LINE__, ack_seq, ov->offload_target_ackd);
				if (TCP_SEQ_GEQ(ack_seq, ov->offload_target_ackd)) {
					// fprintf(stderr, "ProcessACK %d\n", __LINE__);
					int ret;
					uint32_t target_acked_bytes;

					// ACK_SEQ cannot be greater than the next SEQ to send
					assert(TCP_SEQ_GEQ(cur_stream->snd_nxt, ack_seq));

					payloadlen = snprintf(payload, OFFLOAD_META_PAYLOAD_LIMIT,
											"ACKD %u %u", ov->fid, ack_seq);
					...
					if (ret == 0) {
						// fprintf(stderr, "ProcessACK %d\n", __LINE__);
						/* update target ackd
							 unsigned subtraction will get the difference of ack_seq
							 and offload_target_acked, effectively getting the distance
							 between the two */
						target_acked_bytes = ack_seq - ov->offload_target_ackd;
						// increase by OFFLOAD_ACKD_BYTES more
						ov->offload_target_ackd += OFFLOAD_ACKD_BYTES *
								(target_acked_bytes/OFFLOAD_ACKD_BYTES + 1);
				                // fprintf(stderr, "ProcessACK %d updated offload_target_ackd:%d \n", __LINE__, ov->offload_target_ackd);
			...
			// Check if file was already fully ACK-ed
			if (!TCP_SEQ_GT(ov->offload_final_seq, ack_seq) &&
				ov->offload_flags & OFFLOAD_FLAG_CLOSE) {
				payloadlen = snprintf(payload, OFFLOAD_META_PAYLOAD_LIMIT,
										"CLOS %u", ov->fid);
				SendOffloadMetaPacket(mtcp, cur_stream, payload, payloadlen);
				TAILQ_REMOVE(&sndvar->offload_vars_list, ov, offload_vars_link);
				SBUF_LOCK(&mtcp->offload_cleanup_lock);
				TAILQ_INSERT_TAIL(&mtcp->ov_cleanup_list, ov, ov_cleanup_link);
				SBUF_UNLOCK(&mtcp->offload_cleanup_lock);
			}

		}

		if (cur_stream->rcvvar->sacks) {
			ClearSACKTable(cur_stream, ack_seq);
		}
		 ...
		 uint32_t cur_ts, const int ifidx, const struct iphdr *iph, int ip_len)
{
	...
	case TCP_ST_SYN_RCVD:
		...
		if (tcph->syn && seq == cur_stream->rcvvar->irs)
			...
			if (payloadlen > 0 && cur_stream->state == TCP_ST_ESTABLISHED) {
				//fprintf(stderr,"TCP_ST_SYN_RCVD Handle_TCP_ST_ESTABLISHED \n");
	...
	case TCP_ST_ESTABLISHED:
		//TRACE_DBG(stderr,"TCP_ST_ESTABLISHED Handle_TCP_ST_ESTABLISHED \n");
		Handle_TCP_ST_ESTABLISHED(mtcp, cur_ts, cur_stream, tcph, 
...
/*----------------------------------------------------------------------------*/
int
ProccessOffloadEchoPacket(mtcp_manager_t mtcp, 
		 uint32_t cur_ts, const struct iphdr *iph)
{
	...
	if (memcmp(p, "OPEN", 4) == 0) {
		...
		// Update the RTO list with new TS
		...
		if (cur_stream->snd_nxt_echo_set) {
			if (IS_SEQ_GT(ntohl(tcph->seq) + (uint32_t) echo_bytes,
				cur_stream->snd_nxt_echo)) {
				cur_stream->snd_nxt_echo = ntohl(tcph->seq) + (uint32_t) echo_bytes;
