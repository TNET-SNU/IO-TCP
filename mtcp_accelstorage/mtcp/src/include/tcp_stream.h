...
#define MAX_OV_POOL_SIZE 2 // OV per connection
#define MAX_OB_POOL_SIZE 2 // OB per connection

struct offload_vars
{
	TAILQ_ENTRY(offload_vars) offload_vars_link;
	TAILQ_ENTRY(offload_vars) ov_cleanup_link;

	uint8_t offload_ready:1;

	uint32_t fid;
	uint32_t offload_final_seq;
	uint8_t offload_flags;
	uint32_t offload_target_ackd;
...
struct offload_buffer
{
	...
	TAILQ_ENTRY(offload_buffer) ob_cleanup_link;
...
struct tcp_send_vars
{
	...
	TAILQ_ENTRY(tcp_stream) offload_file_link;
	...
	/* Offload Write */
	TAILQ_HEAD (offload_vars_head, offload_vars) offload_vars_list;
	TAILQ_HEAD (offload_buffer_head, offload_buffer) offload_buffer_list;

	uint8_t on_offload_file_list;
};
...
typedef struct tcp_stream
{
	...
	uint32_t snd_nxt_echo;
	uint8_t snd_nxt_echo_set:1;
} tcp_stream;
