...
struct mtcp_sender
{
	...
	TAILQ_HEAD (offload_file_head, tcp_stream) offload_file_list;

	int control_list_cnt;
	int send_list_cnt;
	int ack_list_cnt;
	int offload_file_list_cnt;
};
/*----------------------------------------------------------------------------*/
struct mtcp_manager
{
	...
	stream_queue_t offloadq;				/* streams need to offload */
	...
	uint32_t offload_fid_counter:24;
	mem_pool_t ov_pool;
	mem_pool_t ob_pool;
	TAILQ_HEAD (ov_cleanup_head, offload_vars) ov_cleanup_list;
	TAILQ_HEAD (ob_cleanup_head, offload_buffer) ob_cleanup_list;
#if USE_SPIN_LOCK
	pthread_spinlock_t offload_cleanup_lock;
#else
...
struct mtcp_thread_context
{
	...
	pthread_spinlock_t offloadq_lock;
#else
