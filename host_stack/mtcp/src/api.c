...
int
mtcpstat_to_stat(struct stat * _fc_stat, struct mtcp_stat * mtcp_stat);
/*----------------------------------------------------------------------------*/
static inline int 
...
/*----------------------------------------------------------------------------*/
static inline int 
SetOffloadBuffer(mtcp_manager_t mtcp, tcp_stream *cur_stream, size_t len)
{
	struct tcp_send_vars *sndvar = cur_stream->sndvar;
	uint64_t sndlen;
	size_t ret;
	...
	ret = SBPutEmpty(mtcp->rbm_snd, sndvar->sndbuf, sndlen);
	assert(ret == sndlen);
	sndvar->snd_wnd = sndvar->sndbuf->size - sndvar->sndbuf->len;
	if (ret <= 0) {
		TRACE_ERROR("SBPut failed. reason: %zu (sndlen: %lu, len: %u\n", 
				ret, sndlen, sndvar->sndbuf->len);
		errno = EAGAIN;
		return -1;
	}
...
mtcp_offload_open(mctx_t mctx, int sockid, const char *file_name)
{
	...
	mtcp->offload_fid_counter++;

	if (mtcp->offload_fid_counter > 0x00FFFFFF)
		mtcp->offload_fid_counter = 0;
	...
	if (ret >= 0 &&
		!(sndvar->on_offloadq || sndvar->on_offload_file_list)) {
		SQ_LOCK(&mtcp->ctx->offloadq_lock);
		sndvar->on_offloadq = TRUE;
		StreamEnqueue(mtcp->offloadq, cur_stream); /* this always succeeds */
		SQ_UNLOCK(&mtcp->ctx->offloadq_lock);
	}

	TRACE_API("Stream %d: mtcp_offload_file() returning %d\n",
			  cur_stream->id, ret);
...
mtcp_offload_close(mctx_t mctx, int sockid, int offload_fid)
{
	...
	if (offload_vars == NULL) {
		TRACE_ERROR("Attempting to open file without opening one");
		errno = EINVAL;
		ret = -1;
	} else {
		offload_vars->offload_flags |= OFFLOAD_FLAG_CLOSE;
		ret = 0;
	}

	// TODO(taehyun): remove from fnamestat_table if there is no other connection using this file
	UNUSED(_mfs);
	// strcpy(_mfs.offload_fn, offload_vars->offload_fn);
	// FnameStatHTRemove(mtcp->fnamestat_table, &_mfs);
	
	SBUF_UNLOCK(&sndvar->write_lock);

	TRACE_API("Stream %d: mtcp_offload_close() returning %d\n",
			  cur_stream->id, ret);
...
/*----------------------------------------------------------------------------*/
ssize_t
mtcp_offload_write(mctx_t mctx, int sockid,
				   int offload_fid, off_t *offset, size_t len)
{
	mtcp_manager_t mtcp;
	socket_map_t socket;
	tcp_stream *cur_stream;
	...
	if (ov == NULL) {
		TRACE_API("Offload file not set\n");
		errno = EINVAL;
		SBUF_UNLOCK(&sndvar->write_lock);
		return -1;
	}

	if (ov->offload_flags & OFFLOAD_FLAG_CLOSE) {
		TRACE_API("Offload file is already closed\n");
		errno = EINVAL;
		SBUF_UNLOCK(&sndvar->write_lock);
		return -1;
	}
	...
	if (ov->file_len == 0) {
		// struct mtcp_stat * _mtcp_st;
		// if (-1 == mtcp_offload_fstat(mctx, sockid, offload_fid, &_mtcp_st)) {
		// 	SBUF_UNLOCK(&sndvar->write_lock);
		// 	return -1;
		// }
		// &ov->sb = 
	...
	/* Save a copy of init SEQ before putting in offload buffer */
	if (sndvar->sndbuf)
		init_seq = sndvar->sndbuf->head_seq + sndvar->sndbuf->len;
	else
		init_seq = sndvar->iss + 1;

	ret = SetOffloadBuffer(mtcp, cur_stream, len);

	if (ret > 0) {
		_ob = TAILQ_LAST(&sndvar->offload_buffer_list, offload_buffer_head);

		if (_ob == NULL || _ob->fid != offload_fid
				|| _ob->offset != (off - _ob->len)) {
			ob = (struct offload_buffer *) MPAllocateChunk(mtcp->ob_pool);
			assert(ob);
			if (!ob) {
				TRACE_ERROR("Cannot retrieve memory for offload buffer");
				exit(-1);
			}
			memset(ob, 0, sizeof(*ob));
			ob->fid = offload_fid;
			ob->offset = off;
			TAILQ_INSERT_TAIL(&sndvar->offload_buffer_list,
							  ob, offload_buffer_link);
		} else {
			ob = _ob;
		}

		if (!ob->init_set) {
			ob->head_seq = init_seq;
			ob->init_set = TRUE;
		}

		ob->len += ret;
		ov->offload_final_seq = ob->head_seq + ob->len;
	...
	TRACE_API("Stream %d: mtcp_offload_write() returning %d\n",
			  cur_stream->id, ret);
...
/*----------------------------------------------------------------------------*/
int
mtcp_offload_fstat(mctx_t mctx, const int sockid, const int offload_fid,  struct mtcp_stat** pp_mtcp_stat) {
	mtcp_manager_t mtcp;
	socket_map_t socket;
	tcp_stream *cur_stream;
	struct tcp_send_vars *sndvar;
...
/*----------------------------------------------------------------------------*/
int
mtcp_offload_stat(mctx_t mctx, const char *file_name, struct mtcp_stat** pp_mtcp_stat) {
	mtcp_manager_t mtcp;
...
offload_cleanup(mtcp_manager_t mtcp)
{
	struct offload_vars *ov, *_ov;
	struct offload_buffer *ob, *_ob;

	SBUF_LOCK(&mtcp->offload_cleanup_lock);
	TAILQ_FOREACH_SAFE(ov, &mtcp->ov_cleanup_list, ov_cleanup_link, _ov) {
		TAILQ_REMOVE(&mtcp->ov_cleanup_list, ov, ov_cleanup_link);
		MPFreeChunk(mtcp->ov_pool, ov);
	}
	TAILQ_FOREACH_SAFE(ob, &mtcp->ob_cleanup_list, ob_cleanup_link, _ob) {
		TAILQ_REMOVE(&mtcp->ob_cleanup_list, ob, ob_cleanup_link);
...
mtcpstat_to_stat(struct stat * _fc_stat, struct mtcp_stat * mtcp_stat){
	int ret = 0;
	_fc_stat->st_ino = (ino_t)mtcp_stat->st_ino;
	_fc_stat->st_mode = (mode_t)mtcp_stat->st_mode;
	_fc_stat->st_nlink = (nlink_t)mtcp_stat->st_nlink;
	_fc_stat->st_uid = (uid_t)mtcp_stat->st_uid;
	_fc_stat->st_gid = (gid_t)mtcp_stat->st_gid;
	_fc_stat->st_rdev = (dev_t)mtcp_stat->st_rdev;
	_fc_stat->st_size = (off_t)mtcp_stat->st_size;
	_fc_stat->st_blksize = (long)mtcp_stat->st_blksize;
	_fc_stat->st_blocks = (blkcnt_t)mtcp_stat->st_blocks;
	_fc_stat->st_atime = (time_t)mtcp_stat->_st_atime;
	_fc_stat->st_mtime = (time_t)mtcp_stat->_st_mtime;
	_fc_stat->st_ctime = (time_t)mtcp_stat->_st_ctime;
	return ret;
}