		...
		uint32_t saddr, uint16_t sport, uint32_t daddr, uint16_t dport)
{
	...
	/* Dodie: Initialize offload */
	TAILQ_INIT(&stream->sndvar->offload_vars_list);
	TAILQ_INIT(&stream->sndvar->offload_buffer_list);
	stream->sndvar->on_offload_file_list = FALSE;

	stream->snd_nxt_echo = 0;
	stream->snd_nxt_echo_set = FALSE;
...
DestroyTCPStream(mtcp_manager_t mtcp, tcp_stream *stream)
{
	...
	/* Close offload file if offloaded */
	char payload[OFFLOAD_META_PAYLOAD_LIMIT];
	int payloadlen;
	TAILQ_FOREACH_SAFE(ov, &stream->sndvar->offload_vars_list,
										 offload_vars_link, _o) {
		if (ov->offload_ready) {
			payloadlen = snprintf(payload, OFFLOAD_META_PAYLOAD_LIMIT,
									"CLOS %u", ov->fid);
			ret = SendOffloadMetaPacket(mtcp, stream, payload, payloadlen);
			if(ret < 0)
				TRACE_ERROR("Failed to send offload file close packet\n");
		}
		TAILQ_REMOVE(&stream->sndvar->offload_vars_list, ov, offload_vars_link);
	...
	/* Remove all offload buffers */
	TAILQ_FOREACH_SAFE(ob, &stream->sndvar->offload_buffer_list,
										 offload_buffer_link, _ob) {
		TAILQ_REMOVE(&stream->sndvar->offload_buffer_list,
	...
	/* Remove the stream from the offload file list if in there */
	if (stream->sndvar->on_offload_file_list) {
		struct mtcp_sender *sender;
		if (stream->sndvar->nif_out < 0) {
			sender = mtcp->g_sender;
		}

		int eidx = CONFIG.nif_to_eidx[stream->sndvar->nif_out];
		if (eidx < 0 || eidx >= CONFIG.eths_num) {
			TRACE_ERROR("(NEVER HAPPEN) Failed to find appropriate sender.\n");
			sender = NULL;
		}

		sender =  mtcp->n_sender[eidx];
		if (sender) {
			TAILQ_REMOVE(&sender->offload_file_list, stream,
									 sndvar->offload_file_link);
			sender->offload_file_list_cnt--;
		}
		stream->sndvar->on_offload_file_list = FALSE;
	}
	/* Close offload file END */
